...
var GenerateSurveyReportComponent = /** @class */ (function () {
    function GenerateSurveyReportComponent(route, surveyService, excelService) {
        this.route = route;
    ...
    GenerateSurveyReportComponent.prototype.generateReport = function () {
        ...
        this.questionToReport.forEach(function (question) {
            ...
            pdfContent.push({
                text: 'W sumie na pytanie udzielono łącznie ' + _this.geTotalQuestionScore(question) + ' odpowiedzi.',
                style: ['queryCount']
            });
            // ciało sekcji
            if (question.AnswerType != AnswerType.Tekstowy) { // przeiteruj wszystkie odpowiedzi
                for (var i = 0; i < question.Answers.length; i++) {
                    ...
                    pdfContent.push({
                        text: ' Udzielono ' + _this.getScore(question.QuestionNumber, i + 1) + ' odpowiedzi.',
                        style: ['body']
                    });
                }
            }
    ...
    GenerateSurveyReportComponent.prototype.generateExelReport = function () {
        var _this = this;
        var dataToExel = [];
        var textQuestionToExel = [];
        dataToExel.push([this.survey.Name]);
        this.questionToReport.forEach(function (item) {
            if (item.AnswerType != AnswerType.Tekstowy) {
                dataToExel.push([' '].concat(item.Answers));
                var scores = [];
                for (var index = 0; index < item.Answers.length; index++) {
                    scores.push(_this.getScore(item.QuestionNumber, index + 1));
                }
                dataToExel.push([item.Query].concat(scores));
            }
            else {
                textQuestionToExel.push([item.Query].concat(_this.getAnswers(item.QuestionNumber)));
            }
        });
    ...
    GenerateSurveyReportComponent = __decorate([
        ...
        __metadata("design:paramtypes", [ActivatedRoute, SurveyService, ExcelService])
